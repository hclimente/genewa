...
Most of the network methods, even the consensus, were highly unstable, raising questions about the \caz{veracity}{reliability} of the results. We built a \caz{}{new, stable} consensus network using the 30 solutions \caz{from}{obtain by running} the 6 different methods on 5 different splits \caz{}{each} containing 80\% of the data (Section \ref{methods:comparison}). Such a network \caz{would}{is expected to} capture the subnetworks more often found altered, and hence should be more resistant to noise. We used only genes selected in at least 7 of the settings (\cazcom{percentile 99\%}{I'm reluctant to talk about percentiles here, as the genes aren't ranked with each a different value -- I'd rather say ``corresponding to selecting 1\% of the genes'' or something like that}). The resulting stablility-based consensus was composed of 68 genes \cazcom{}{Figure??? Also add  a figure with all gene names in the supplementaries, like Supp Fig 4?}. This network shares most of the proterties of the consensus: breast cancer susceptibility genes are overrepresented (P-value = 3 \texttimes{} 10\textsuperscript{-4}), as well as genes involved in mitochondrial translation and the attenuation phase (adjusted P-values 0.001 and 3 \texttimes{} 10\textsuperscript{-5} respectively); the selected genes are more central than average (P-value = 1.1 \texttimes{} 10\textsuperscript{-14}); and \cazcom{19 of them are isolated}{what property of the previous consensus network is this?}.

However, \caz{the shared global properties of the network}{although this new network exhibits similar global properties as the previous one}, the lack of stability results in different genes being selected. In this case, the most central gene of the network is \emph{CUL3}, which is absent from the \caz{}{previous} consensus network. Indeed, \emph{CUL3} has a low association score in both GENESIS and BCAC (P-values of 0.04 and 0.26, respectively). \emph{CUL3} is a component of Cullin-RING ubiquitin ligases. It impacts the protein levels of multiple genes relevant for cancer progression \cite{Chen2016}, and its overexpression was also linked to increased sensitivity to carcinogens \cite{Loignon2009}. 

\section{Discussion}

In recent years, the ability of GWAS to unravel the mechanisms leading to complex diseases has been called into question \cite{boyle_expanded_2017}. The omnigenic model proposes that gene functions are interwoven with each other in a dense co-function network. The practical consequence is that larger and larger GWAS will lead to the discovery of an uninformative wide-spread pleiotropy. \caz{Other than such problems}{In addition}, discovery in GWAS is hindered by a conservative statistical framework. Network methods tackle \caz{both}{these two} issues by using both the association score and an interaction network to \caz{consider}{take into consideration} the biological context of each of the genes and SNPs. Based on what could be considered diverse interpretations of the omnigenic model, several methods for network-guided biomarker discovery have been proposed in recent years. In this article we evaluate the relevance of six network methods by examining a GWAS dataset on familial breast cancer.

Most of these methods produced a relevant subset of biomarkers, recapitulating known breast cancer susceptibility genes. \caz{On median}{In general} \cazcom{}{I don't think ``On median'' exists, unfortunately}, the selected genes and SNPs were more central than average, in accordance with the observation that disease genes are more relatively central \cite{pinero_uncovering_2016}. However, very central nodes are also more likely to be connecting a given random pair of nodes, making them more likely to be selected by these network methods. Hence, further work is needed to characterize the impact of centrality on these methods' outputs. Despite their similarities, the solutions were notably different. At one end of the spectrum, SConES and heinz preferred small, highly associated solutions, providing a conveniently short list of biomarkers, at the expense of not shedding much light on the etiology of the disease. On the other end, SigMod and dmGWAS gravitate towards larger, less associated solutions which provide a wide overview of the biological context. While this deepens our understanding of the disease and provide biological hypotheses, they require further analyses, which might deter unexperienced practitioners. HotNet2 balances both approaches at the expense of producing the largest solution: a constellation of many, highly associated, small subnetworks. Despite these differences, all solutions are equally bad at discriminating cases from controls. In addition, all methods are remarkably unstable, yielding different solutions for slightly different inputs. This might partly be caused by the instability of the P-values themselves in low statistical power settings \cite{halseyFickleValueGenerates2015}. Hence, heinz's conservative transformation of P-values, which favors only the most extreme ones, leads to improved stability. Another source of instability might be the redundancy inherent to biological networks. \cazcom{We built a stable consensus, which accounts for that instability. Encouragingly, this network shared the same biological and topological properties as the individual solutions, despite the few genes in common due to instability. Overall, this supports that instability might be caused by redundant but equivalent biological mechanisms, and hence it validates the conclusions obtained on the individual solutions and the consensus.}{This comes too early, the next paragraph discusses why and how we built the consensus network and this bit should only arrive afterwards}
