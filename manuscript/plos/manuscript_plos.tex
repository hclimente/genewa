...
\title{Comparing and combining network-guided biomarker discovery to discover genetic susceptibility mechanisms to breast cancer on the GENESIS study}
\author{Héctor Climente-González$^{1,2,3,4,\spadesuit}$, Christine Lonjou$^{1,2,3}$, Fabienne Lesueur$^{1,2,3}$, \\
Dominique Stoppa-Lyonnet$^{5,6,7,\clubsuit}$, Nadine Andrieu$^{1,2,3}$, Chloé-Agathe Azencott$^{3,1,2}$}
\date{\small{
  \raggedright{
  ...
  $^{\clubsuit}$For the GENESIS study group\\
  $^{\spadesuit}$Corresponding author: \href{hector.climente@riken.jp}{hector.climente@riken.jp}\\
  }
  }
}
\hypersetup{
...
\raggedright{\textbf{Author summary}}
