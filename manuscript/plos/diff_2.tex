...
%DIF DEL review_1.tex   Mon Jan 18 12:03:11 2021
%DIF ADD review_2.tex   Mon Jan 18 12:04:06 2021
...
\DIFdelend \DIFaddbegin \newline{How can biological networks boost GWAS? A case study on susceptibility to familial breast cancer} \DIFaddend % Please use "sentence case" for title and headings (capitalize only the first word in a title (or heading), the first word in a subtitle (or subheading), and any proper nouns).
}
\newline
...
\section*{Abstract}
\DIFdelbegin \DIFdel{Network approaches to disease use biological networks, which model functional relationships between the molecules in a cell, to generate hypotheses about the genetics }\DIFdelend \DIFaddbegin \DIFadd{Genome-wide association studies (GWAS) explore the genetic causes }\DIFaddend of complex diseases. \DIFdelbegin \DIFdel{Several among them jointly consider gene scores, representing the association between each gene and the disease, and the }\DIFdelend \DIFaddbegin \DIFadd{However, they ignore the }\DIFaddend biological context of \DIFdelbegin \DIFdel{each gene, modeled by a network . Here, we study six such network methods using gene scores from }\DIFdelend \DIFaddbegin \DIFadd{the SNPs and genes under study. To address this shortcoming, one can seek susceptibility genes that are functionally related, employing networks to integrate such functional information. Many such network methods exist, each stemming from different mathematical frameworks, pre-processing steps, and assumptions about the susceptibility mechanism. Unsurprisingly, their results are also disparate. To sort through this diversity, we selected six network methods and applied them to }\DIFaddend GENESIS, a \DIFdelbegin \DIFdel{genome-wide association study (GWAS ) }\DIFdelend \DIFaddbegin \DIFadd{GWAS }\DIFaddend on French women with non-BRCA familial breast cancer. \DIFdelbegin \DIFdel{We provide a critical comparison of these six methods, discussing the impact of their mathematical formulation and parameters. Using a biological network yields more compelling results than standard GWAS analyses. Indeed, we find significant overlaps between our solutions and the genes identified in the largest GWAS on breast cancer susceptibility. We further propose to combine these solutions into a consensus network, which brings further insights. The consensus network contains }\DIFdelend \DIFaddbegin \DIFadd{First, we verified that network methods recovered more interesting and interpretable genes than a standard GWAS. Nevertheless, as expected, these results were notably different. We addressed this heterogeneity studying their overlap, computing what we called the }\emph{\DIFadd{consensus}}\DIFadd{. The key gene in this consensus solution was }\DIFaddend \emph{COPS5}, a gene related to multiple \DIFdelbegin \DIFdel{hallmarks of cancer , and 14 of its neighbors. The main }\DIFdelend \DIFaddbegin \DIFadd{cancer hallmarks. Another }\DIFaddend drawback of network methods \DIFdelbegin \DIFdel{is that they are not robust to small perturbations in their inputs}\DIFdelend \DIFaddbegin \DIFadd{was their instability, selecting very different genes on different subsamples of GENESIS}\DIFaddend . Therefore, we \DIFdelbegin \DIFdel{propose a stable consensus solution , }\DIFdelend \DIFaddbegin \DIFadd{proposed a }\emph{\DIFadd{stable consensus}} \DIFadd{solution }\DIFaddend formed by the \DIFaddbegin \DIFadd{68 genes }\DIFaddend most consistently selected \DIFdelbegin \DIFdel{genes in multiple subsamplesof the data. In GENESIS, it is composed of 68 genes, enriched in known }\DIFdelend \DIFaddbegin \DIFadd{across multiple subsamples. Importantly, this solution was also enriched in genes associated with }\DIFaddend breast cancer susceptibility \DIFdelbegin \DIFdel{genes }\DIFdelend (\emph{BLM}, \emph{CASP8}, \emph{CASP10}, \emph{DNAJC1}, \emph{FGFR2}, \emph{MRPS30}, and \emph{SLC4A7}, P\=/value~=~$3 \times 10^{-4}$)\DIFdelbegin \DIFdel{and occupying more central positions in the network than most genes. The network is organized around }\DIFdelend \DIFaddbegin \DIFadd{. The most connected gene was }\DIFaddend \emph{CUL3}, \DIFdelbegin \DIFdel{which is involved in the regulation }\DIFdelend \DIFaddbegin \DIFadd{a regulator }\DIFaddend of several genes linked to cancer progression. \DIFdelbegin \DIFdel{In conclusion, we showed how network methods help overcome the lack of statistical power of GWAS and improve their interpretation. Project-agnostic }\DIFdelend \DIFaddbegin \DIFadd{Lastly, we evaluated the biases of each method and the impact of the parameters on the outcome. In general, network methods preferred highly connected genes, even after random rewirings that stripped the connections of any biological meaning. In conclusion, we presented the advantages of network-guided GWAS, characterized their shortcomings, and provided strategies to address them. To compute the consensus networks, }\DIFaddend implementations of all \DIFaddbegin \DIFadd{six }\DIFaddend methods are available at \url{https://github.com/hclimente/gwas-tools}.
...
\paragraph*{\DIFdelbegin \DIFdel{S2 }\DIFdelend \DIFaddbegin \DIFadd{S4 }\DIFaddend Table.}
\label{stab:sigmod_pwy}
\textbf{Pathway enrichment analyses of the genes in SigMod's solution.}

\paragraph*{\DIFdelbegin \DIFdel{S3 }\DIFdelend \DIFaddbegin \DIFadd{S5 }\DIFaddend Table.}
\label{stab:dmgwas_pwy}
\textbf{Pathway enrichment analyses of the genes in dmGWAS' solution.}

\paragraph*{\DIFdelbegin \DIFdel{S4 }\DIFdelend \DIFaddbegin \DIFadd{S6 }\DIFaddend Table.}
...
%DIFAUXCMD
%DIFDELCMD < \label{sfig:rewirings}
%DIFDELCMD < %%%
