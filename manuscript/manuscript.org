...
In this context, many authors turn to network biology to handle the complex interplay of biomolecules that lead to disease cite:furlong_human_2013. As the name suggests, network biology models biology as a network, where the genes are nodes, and the edges are selected functional relationships between them. These functional relationships come from evidence that the genes jointly contribute to a biological function; for instance, their expression is correlated, or a protein-protein interaction between their gene products have been discovered. An examination of such networks shows that disease genes are more connected than other genes cite:pinero_uncovering_2016. This was particularly true for cancer driver genes, which tended to be key players in connecting different densely-connected communities of genes. 
...
Additionally, any gene that was selected by one of the networks, and that is currently used as to assess breast cancer susceptibility was added to the network. We used the 153 genes associated with familial breast cancer in DisGeNET cite:pinero_disgenet:_2017. 
...
** TODO Network methods produced heterogeneous results
...
We applied six network methods to the GENESIS dataset (Section [[#methods:methods]]). We obtained eight solutions (Supplementary files 1 and 2): one for each of the gene-based methods (Section [[#methods:gene_network]]), and one for each of the SNP networks of SConES (Section [[#methods:snp_network]]). The solutions were very heterogeneous (Table [[tab:solutions]]): none of the subnetworks examined by LEAN was significant (adjusted P-value < 0.05), while dmGWAS produced the largest solution subnetwork with 194 genes. In fact, the overlap between the solution subnetworks is low (Supplementary figure [[sfig:jaccard_methods]]). The methods also differed in the association of the selected genes with the disease. In our experiments, we found SConES GS and SConES GM to favor highly associated genes, while 
...
** TODO The consensus subnetwork recovers known genes 

#+CAPTION:Consensus subnetwork on GENESIS (Section [[#methods:consensus]]). The labeled genes have a VEGAS2 P-value < 0.001 and/or are a known to be associated to familial breast cancer in DisGeNET cite:pinero_disgenet:_2017 (colored in pink).
#+NAME: fig:consensus
[[./figures/figure_3.pdf]]

We built a consensus subnetwork that captures the mechanisms most shared among the subnetworks from the different methods (Figure [[fig:consensus]], Section [[#methods:consensus]]). It contained 53 genes. Due to the low overlap between the methods, only 6 gene were common only to more than two methods. Three of them are associated to familial breast cancer in DisGeNET (BLM, CASP8, TOX3), three are not (SLC4A7, RAB6A, OFD1). Among the remaining 47 genes, 5 of them are also associated to familial breast cancer on DisGeNET (BABAM1, CENPE, FGFR2, MRE11, NBN). Overall, the consensus subnetwork is enriched in DisGeNET familial breast cancer genes (Fisher's exact test P-value = 1.39 \times 10^{-10}). Driven by those, the consensus subnetwork displayed an enrichment in DNA helicase activity (GO Enrichment Analysis adjusted P-value = 0.01).

unfolded protein binding (GO Enrichment Analysis adjusted P-value = 0.01)
poly(U) RNA binding (GO Enrichment Analysis adjusted P-value = 0.03)
structural constituent of cytoskeleton (GO Enrichment Analysis adjusted P-value = 0.03)
...
*** Peripheral genes are handicapped 
